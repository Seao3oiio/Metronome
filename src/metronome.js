...
export function playbackVisualMarkers({ bar, beat, sub, gap }, editorBarIndex) {
  if (gap) return [];
  const markers = [
    { target: "stage-beat", value: beat, className: "is-active" },
    { target: "bar-preview", value: bar, className: "is-playing" },
    { target: "preview-step", value: `${bar}:${beat}:${sub}`, className: "is-playing" },
  ];
  if (bar === editorBarIndex) {
    markers.push({
      target: "editor-step",
      value: `${beat}:${sub}`,
